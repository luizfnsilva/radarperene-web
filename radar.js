...
(function () {
  ...
  function render(node, d, lang, sections, chrome) {
    ...
    if (show("intermercado") && rr.intermercado_br && rr.intermercado_br.length) { h += '<h4>' + (L ? "BR intermarket (stocks)" : "Intermercado BR (bolsa)") + '</h4><div class="g3">' +
      rr.intermercado_br.map(function (x) { var hasTk = x.tickers && x.tickers.length, xp = x.fonte || hasTk; return '<div class="t ' + esc(x.tom) + '"' + (xp ? ' data-exp="1"' : '') + '><div class="n">' + esc(x.nome) + (xp ? ' <span class="rr" style="opacity:.55">＋</span>' : '') + '</div><div class="rr" style="margin-top:4px">' + esc(x.leitura) + '</div>' + (xp ? '<div class="more">' + (x.fonte ? '<div class="mi">' + esc(x.fonte) + '</div>' : '') + (hasTk ? '<div class="mi">' + x.tickers.map(function (tk) { return esc(tk.ticker) + (tk.dy != null ? ' <span style="opacity:.6">DY ' + esc(tk.dy) + '%</span>' : ''); }).join(' · ') + '</div>' : '') + '</div>' : '') + '</div>'; }).join("") + '</div>'; }
