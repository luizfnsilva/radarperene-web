...
(function () {
  ...
  function injectStyle() {
    ...
    s.textContent =
      ...
      ".rp .rc{padding:14px 2px 16px;border-bottom:1px solid var(--_line)}.rp .rk{font-size:9.5px;letter-spacing:.16em;text-transform:uppercase;color:var(--_dim);font-weight:700}.rp .rg{font-size:clamp(18px,3.2vw,26px);font-weight:700;line-height:1.05;letter-spacing:-.01em;text-transform:uppercase;color:var(--_txt);margin:7px 0 6px}.rp .rv{font-family:var(--_numf);font-size:13px;color:var(--_dim)}.rp .rv .ru{opacity:.5}" +
  ...
  function render(node, d, lang, sections, chrome, skin) {
    ...
    // mini-capa editorial (regime BR/Global/Intermercado): rótulo eyebrow + ESTADO grande (a "manchete") + score discreto — "capa de revista", não card de dashboard
    function rcard(k, sc, r) { var val = (sc == null) ? (GATED ? glock() : "—") : (esc(sc) + '<span class="ru">/100</span>'); return '<div class="rc"><div class="rk">' + esc(k) + '</div><div class="rg">' + esc(r || "—") + '</div><div class="rv">' + val + '</div></div>'; }
    ...
    if (show("regime") && rr.regime) { var g = rr.regime; h += '<h4>' + (L ? "Current signal · regime" : "Sinal atual · regime") + ' <span style="font-size:10px;letter-spacing:.04em;color:var(--_dim);font-weight:600;border:1px solid var(--_line);border-radius:5px;padding:1px 6px;vertical-align:middle">' + (L ? "MONTHLY" : "MENSAL") + '</span></h4><div class="legend">' + (L ? "0–100 · 50 ≈ neutral · higher = more risk/pressure · the regime is monthly (moves at month-end)" : "0–100 · 50 ≈ neutro · quanto maior, mais risco/pressão · o regime é mensal (muda no fecho do mês)") + '</div><div class="g3">' +
      rcard(L ? "Brazil" : "Brasil", (g.brasil || {}).score, (g.brasil || {}).regime) + rcard("Global", (g.global || {}).score, (g.global || {}).regime) +
      rcard(L ? "BR intermarket" : "BR intermercado", (g.br_intermercado || {}).score, (g.br_intermercado || {}).regime) + '</div>';
