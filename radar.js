...
(function () {
  ...
  // ★ linha de upsell de PROFUNDIDADE (regra: free mostra a conclusão, Founder a distribuição) — usada no radar completo e no ticket.
  function rpUpsell(L, txt) { return '<div class="rp-ml" style="margin-top:7px;opacity:.9">' + lockA(L, '<span style="color:var(--_accent)">🔒</span> ' + txt) + '</div>'; }
  ...
  function openBig(s, title, meta, lang, fund, preCmp, _retry) {
    ...
    if (s.stats && isDiff) { var stD = s.stats, ppd = ppDeltas(s);
      ...
      // ★ free = AMOSTRA: só a conclusão 3M; a tira de stats (retornos completos · faixa 52s · vol/drawdown · Sharpe) é commodity Bloomberg → Founder. O diferencial (análogo, regime, ânima, risco) fica ao lado.
      var labs = gpaid ? (st.monthly ? [["m1", "1m"], ["m3", "3m"], ["m6", "6m"], ["y1", "12m"]] : [["d1", "1d"], ["w1", "1sem"], ["m1", "1m"], ["m3", "3m"], ["m6", "6m"], ["y1", "12m"]]) : [["m3", "3m"]];
      ...
      if (cells) depth += '<div class="rp-ml" style="margin-top:9px"><b>' + (L ? "Returns" : "Retornos") + '</b></div><div class="rp-rc">' + cells + '</div>' + (gpaid ? '' : rpUpsell(L, L ? "full return series in Founder →" : "retornos completos no Founder →"));  // ★ retornos como cards (briefing: cara de laboratório, não tira de texto)
      if (gpaid) {  // tira de estatística (faixa 52s · vol/drawdown · Sharpe) = só Founder
    ...
    if (gpaid) h += oscTextLine(s.hist2, s.hist2_label, lang) + oscTextLine(s.hist3, s.hist3_label, lang);  // ★ volume/FnG/TVL (domínio) = indicador, não gancho → só Founder
    ...
    // free: Regime + Humor (a CONCLUSÃO legível); Tendência X/10 + vs IBOV (score operacional) = Founder.
    if (((riscoOk || animaOk) || (gpaid && trendBlk)) && !(preCmp && preCmp.length >= 2)) {
      ...
      h += lr + (gpaid ? trendBlk : '');  // trendBlk = Tendência X/10 + vs IBOV Y/10 (operacional → Founder)
    }
  ...
  function rpTeseHTML(rr, L) {
    ...
    var teMore = '<div class="more">' +
      ...
      (te.desfecho ? '<div class="mi"><b>' + (L ? "Outcome — " : "Desfecho — ") + '</b>' + esc(te.desfecho) + '</div>' : '') +
    ...
    return '<h4>' + (te.consumada ? (L ? "Consummated thesis · from signal to law" : "Tese consumada · do sinal à lei") : (L ? "Live thesis · why it was calculated" : "Tese viva · por que foi calculada")) + '</h4>' +
  ...
  function render(node, d, lang, sections, chrome, skin) {
    ...
    var pro = rpIsPro();  // ★ free = AMOSTRA (~40%, "excesso controlado"): mostra as CONCLUSÕES; a profundidade (distribuições, séries longas, lead-lag) é Founder. Caps client-side p/ conversão; o moat real segue server-side (digest/serie por token).
    function upsell(txt){ return rpUpsell(L, txt); }
    function capList(arr, n, klass, proMore, freeNoun){  // pro: lista expansível; free: corta em n + linha de upsell
      if (pro) return collapseList(arr, n, klass, proMore);
      if (arr.length <= n) return '<div class="' + klass + '">' + arr.join("") + '</div>';
      return '<div class="' + klass + '">' + arr.slice(0, n).join("") + '</div>' + upsell((arr.length - n) + ' ' + freeNoun);
    }
    ...
    if (pro && show("scatter") && rr.regime_scatter && rr.regime_scatter.points) { var sct = rr.regime_scatter; var dist = distChart(sct);  // ★ nuvem+histograma = 2ª lente (distribuição) do Análogo BR → Founder; free fica com a conclusão (o Análogo BR acima)
    ...
    if (show("termometros") && v.termometros) { var tms = v.termometros.slice().sort(function (p, q) { return Math.abs((q.valor == null ? 50 : q.valor) - 50) - Math.abs((p.valor == null ? 50 : p.valor) - 50); });
      ...
      capList(tCards, 3, "g3", L ? "more thermometers" : "termômetros", L ? "more thermometers in Founder →" : "termômetros no Founder →") +
      ((!sections && pro) ? '<button type="button" class="rp-explore" data-tab="termometros" style="margin-top:8px">' + (L ? "All " + tms.length + " thermometers" : "Todos os " + tms.length + " termômetros") + ' <span class="a">→</span></button>' : ''); }
    if (show("cripto") && v.cripto && v.cripto.length) { h += '<h4>' + (L ? "Crypto · highlights" : "Cripto · destaques") + '</h4>' + (v.cripto_sentimento ? '<div class="legend">Fear &amp; Greed: ' + esc(v.cripto_sentimento.fng) + ' (' + esc(v.cripto_sentimento.leitura) + ')</div>' : '') + (pro && v.cripto_onchain ? '<div class="legend">' + esc(v.cripto_onchain.nota) + ': ' + [v.cripto_onchain.tvl ? 'TVL ' + esc(v.cripto_onchain.tvl) : '', v.cripto_onchain.stablecoin ? 'stablecoins ' + esc(v.cripto_onchain.stablecoin) : '', v.cripto_onchain.ssr != null ? 'SSR ' + esc(v.cripto_onchain.ssr) : ''].filter(Boolean).join(' · ') + '</div>' : '') + '<div class="tk">' +  // ★ free = 3 moedas + Fear&Greed (a CONCLUSÃO de sentimento); o on-chain TVL/stablecoin/SSR (operacional) = Founder
      (pro ? v.cripto : v.cripto.slice(0, 3)).map(function (t) { return '<span class="i" data-cod="' + esc(String(t.simbolo).toLowerCase()) + '" data-cls="cripto"><span class="sy">' + esc(t.simbolo) + '</span><span class="pr">$ ' + esc(t.preco) + '</span>' + (t.pos52 != null ? '<span class="mt">' + esc(t.pos52) + (L ? "% of 52w" : "% da faixa 52s") + '</span>' : '') + '</span>'; }).join("") + '</div>'; }
    if (pro && show("extras")) { var ex = [];  // ★ breadth BR 36m / geográfico = mecânica interna (série, não gancho) → Founder
    ...
    if (pro && show("leadlag") && v.lead_lag && v.lead_lag.length) { h += '<h4>' + (L ? "Lead-lag · statistically significant (FDR)" : "Lead-lag · com significância (FDR)") + '</h4>' +  // ★ joia do Vértice (antecedência estatística) = Founder inteiro
    ...
    if (show("divergencias") && v.divergencias && v.divergencias.length) { h += '<h4>' + (L ? "Divergences today" : "Divergências hoje") + '</h4><ul class="dv">' +
      (pro ? v.divergencias : v.divergencias.slice(0, 1)).map(function (x) { return '<li><b>' + esc(x.codigo) + '</b> · ' + esc(x.leitura) + '</li>'; }).join("") + '</ul>' +
      (pro || v.divergencias.length <= 1 ? '' : upsell((v.divergencias.length - 1) + (L ? " more divergences in Founder →" : " divergências no Founder →"))); }
    // ★ Vértice free: termômetros (3) + cripto + 1 divergência são as CONCLUSÕES; a estrutura estatística (lead-lag, estudo de análogo, breadth 36m) é Founder.
    if (!pro && !sections) h += upsell(L ? "lead-lag, analog studies & the full statistical structure in Founder →" : "lead-lag, estudos de análogo & a estrutura estatística completa no Founder →");
