...
(function () {
  ...
  function oscCaption(rk, lang, kind) {
    ...
    return (L ? "BR regime: " : "Regime BR: ") + pos + (L ? " · ticks mark past extremes" : " · traços marcam extremos passados");
  }
  ...
  function openBig(s, title, meta, lang, fund, preCmp) {
    ...
    if (s.stats) { var st = s.stats;
      ...
      h += '<div class="rp-ml" style="margin-top:6px"><b>' + (L ? "Volatility " : "Volatilidade ") + st.vol + '%</b> ' + (L ? "(annualized)" : "(anualizada)") + (st.dd_top != null ? ' · ' + (L ? "drawdown from peak " : "queda do topo ") + '<b style="color:var(--_cool)">' + st.dd_top + '%</b>' : '') + '</div>';
  ...
  function renderAtivo(node, codigo, classe, lang, skin) {
    ...
    fetch(API.replace("/v1/digest", "/v1/serie") + "?codigo=" + encodeURIComponent(codigo) + "&classe=" + encodeURIComponent(classe), FOPT)
      .then(function (r) { return r.json(); }).then(function (s) {
        ...
        if (gpaid && s.fair && s.fair.premio_pct != null) { var isFii = s.fair.tipo === "fii"; h += '<div class="rp-ml" style="margin-top:8px">' + (isFii ? (L ? "Net asset value " : "Valor patrimonial ") : (L ? "Fair value " : "Valor-justo ")) + '<b style="color:var(--_warm)">' + (s.fair.premio_pct >= 0 ? "+" : "") + esc(s.fair.premio_pct) + '%</b> ' + (L ? "vs price" : "vs preço") + (isFii ? ' · P/VP ' + esc(s.fair.pvp) : '') + '</div>'; }
